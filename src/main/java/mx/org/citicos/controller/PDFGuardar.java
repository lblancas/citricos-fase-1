...
public class PDFGuardar implements Runnable {
	...
	public PDFGuardar(int id, int opcion, int usuario, String fileName) {
		super();
		this.id = id;
		this.opcion = opcion;
	...
	public void run() {
		...
		if (controller.impresionXusuario(getUsuario())) {
			guardarPallet(folio.getId(), folio.getFolio(), folio.getCodigo(), folio.getFecha(), folio.getProductor(),
					folio.getPeso_bruto(), folio.getPeso_tara(), folio.getPeso_neto(), folio.getNo_rejas(),
					folio.getTipos_rejas(), folio.getTipos_limones(),
					(folio.getAgronomo() != null ? folio.getAgronomo() : ""), folio.getObservaciones(), folio.getDejo(),
					folio.getJapon(), folio.getSegundas(), folio.getTerceras(), folio.getTorreon(), folio.getColeada(),
					fechaModificacion, folio.getTicket(), getOpcion(), getUsuario(), folio.getId(), fileName);
		}

	}

	private static void guardarPallet(Integer Id, String Folio, String Codigo, String Fecha, String Productor,
			Double bruto, Double tara, Double neto, Double rejas, String trejas, String tlimon, String agronomo,
			String observaciones, int dejo, Double Japon, Double Segundas, Double Terceras, Double Torreon,
			Double Coleada, String fechaModificacion, String ticket, int opcion, int usuario, int idTicket,
			String fileName) {
		try {
			ByteArrayOutputStream documentoBytes = null;
			System.out.print("!");
			documentoBytes = crearDocumentoiText(Folio, Codigo, Fecha, Productor, bruto, tara, neto, rejas, trejas,
					tlimon, agronomo, observaciones, dejo, Japon, Segundas, Terceras, Torreon, Coleada,
					fechaModificacion, ticket, opcion, idTicket);
			System.out.print("*");
	...
	private static Table creaTabla(Double rejas, String trejas, String tlimon, String agronomo, String observaciones,
			Image dejo, String ticket) {
	...
	private static Table creaTabla(Double Japon, Double Segundas, Double Terceras, Double Torreon, Double Coleada) {
	...
	private static Table creaTabla(String c1, String c2, String c3, String c4, String c5, String c6, String c7,
			String c8, String c9, String c10, String c11, String c12, Image img1, String fecha, Image img2, String dato,
			Image limon) {
		Table table1 = new Table(3);
		table1.setBorder(Border.NO_BORDER);
		table1.setWidth(500F);
	...
	private static ByteArrayOutputStream crearDocumentoiText(String Folio, String Codigo, String Fecha,
			String Productor, Double bruto, Double tara, Double neto, Double rejas, String trejas, String tlimon,
			String agronomo, String observaciones, int dejo, Double Japon, Double Segundas, Double Terceras,
			Double Torreon, Double Coleada, String fechaModificacion, String ticket, int opcion, int idTicket) {
		ByteArrayOutputStream documentoBytes = null;
		try {
			...
			if (tlimon != null) {
				if (tlimon.toUpperCase().indexOf("EMP") >= 0)
					imagen_calidad_limon += "empaque.png";
				else if (tlimon.toUpperCase().indexOf("REV") >= 0)
					imagen_calidad_limon += "revuelto.png";
				else if (tlimon.toUpperCase().indexOf("VER") >= 0)
					imagen_calidad_limon += "verde.png";
				else if (tlimon.toUpperCase().indexOf("ORG") >= 0)
					imagen_calidad_limon += "organico.png";
			}
			...
			documento.add(
					new Paragraph("Fecha de Impresi\u00f3n: " + fechaModificacion + "  // Ticket [" + idTicket + "]"));
			documento.add(creaTabla("Folio", Folio, "       " + Codigo, "Productor", Productor, "       *",
					"Peso Bruto", "Peso Tara", "Peso neto", "" + bruto, "" + tara, "" + neto, img1,
					"Fecha[" + Fecha + "]", img2, dato, imgLimon));
			if (opcion == 2 || opcion == 3) {
				...
				documento.add(creaTabla(rejas, trejas, tlimon, agronomo, observaciones, imagen, ticket));
			}
			if (opcion == 3) {
				documento.add(new Paragraph("   "));
